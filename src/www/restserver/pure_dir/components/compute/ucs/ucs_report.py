...
def get_ucs_system_cluster_info(args={}):
    ...
    try:
        ...
        ucs_ini = {'name' : "", 'ip' : "", 'ucsm_version' : "", 'chassis_discovery' : "", 'power_redundancy' : "", 
                   'mgmt_interface_monitoring' : "", 'eth_switchmode' : ""}
        ...
        ucs_cluster['mgmt_interface_monitoring'] = handle.query_dn("sys/extmgmt-intf-monitor-policy").admin_state
        ...
        if [ucs_dict for ucs_dict in ucs_cluster_list if (cmp(ucs_dict, ucs_ini)) != 0] != []:
...
def get_ucs_system_fi_info(args={}):
    ...
    try:
        ...
        for ucs_info in handle.query_classid(class_id="NetworkElement"):
            ucs_ini = {'fabric_name' : "", 'fabric_ip' : "", 'fabric_subnetmask' : "", 'fabric_gateway' : "", 'model' : "",
                       'serial' : "", 'leadership' : "", 'nxos_system' : "", 'nxos_kickstart' : ""} 
        ...
        if [ucs_dict for ucs_dict in ucs_fiinfo_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_fi_inventory(args={}):
    ...
    try:
        ...
        for ucs_fiinv in handle.query_classid(class_id="NetworkElement"):
            ucs_ini = {'fabric_name' : "", 'dn' : "", 'model' : "", 'description' : "", 'operability' : "", 'state' : "", 
                       'serial' : ""}
        ...
        if [ucs_dict for ucs_dict in ucs_fiinventory_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_fi_globalsettings(args={}):
    ...
    try:
        ...
        for fi_global in handle.query_classid(class_id="CommSvcEp"):
            ucs_ini = {'domain' : "", 'timezone' : "",'snmp_community' : "", 'dns': [], 'ntp_server': [], 
                       'syslog_server': [], 'snmp_trapreceiver': []}
			...
			if len(handle.query_children(in_mo=fi_globchild)) != 0:
                            mo_global_child = handle.query_children(in_mo=fi_globchild, class_id = "CommDnsProvider")
		    ...
		    if fi_globchild.get_class_id() == "CommDateTime":
                        ...
                        if len(handle.query_children(in_mo=fi_globchild)) != 0:
                            mo_global_child = handle.query_children(in_mo=fi_globchild, class_id = "CommNtpProvider")
			...
			if len(handle.query_children(in_mo=fi_globchild)) != 0:
                            mo_global_child = handle.query_children(in_mo=fi_globchild, class_id = "CommSyslogClient")
                            for fi_syschild in mo_global_child:
                                if fi_syschild.get_class_id() == "CommSyslogClient":
                                    ucs_figlobalset['syslog_server'].append(fi_syschild.hostname + "/" +
                                        fi_syschild.forwarding_facility + "/" + fi_syschild.severity)
			...
			if len(handle.query_children(in_mo=fi_globchild)) != 0:
                            mo_global_child = handle.query_children(in_mo=fi_globchild, class_id = "CommSnmpTrap")
                        for fi_snmpchild in mo_global_child:
                            if fi_snmpchild.get_class_id() == "CommSnmpTrap":
                                ucs_figlobalset['snmp_trapreceiver'].append(fi_snmpchild.hostname + "/" +
                                    fi_snmpchild.community)
            ucs_figlobalset_list.append(copy.deepcopy(ucs_figlobalset))
        if [ucs_dict for ucs_dict in ucs_figlobalset_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_organizations(args={}):
    ...
    try:
        ...
        if [ucs_dict for ucs_dict in ucs_sysorg_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_callhomebase(args={}):
	...
	for ucs_syscall in handle.query_classid(class_id="CallhomeEp"):
            ucs_ini = {'admin_state' : "", 'inventory' : "", 'contact_details' : {'contactname' : {"label": "Contact Name", "value": ""},
                       'contactphone' : {"label": "Contact Phone", "value": ""}, 'contactemail' : {"label": "Contact Email", "value": ""}, 
                       'contactaddress' : {"label": "Contact Address", "value": ""}, 'contractno' : {"label": "Contract No", "value": ""}, 
                       'customerno' : {"label": "Customer No", "value": ""}, 'siteno': {"label": "Site No", "value": ""}},
                       'mail_details' : {'emailfrom' : {"label": "Email From", "value": ""}, 'emailto' : {"label": "Email To", "value": ""}}}
            ...
            if len(mo_homebase) != 0:
                for ucs_syscall_child in mo_homebase:
                    ...
                    if ucs_syscall_child.get_class_id() == "CallhomeSource":
                        ucs_callhome['contact_details'] = {'contactname': {"label": "Contact Name", "value": ucs_syscall_child.contact},
                                                           'contactphone': {"label": "Contact Phone", "value": ucs_syscall_child.phone},
                                                           'contactemail': {"label": "Contact Email", "value": ucs_syscall_child.email},
                                                           'contactaddress': {"label": "Contact Address", "value": ucs_syscall_child.addr},
                                                           'contractno': {"label": "Contract No", "value": ucs_syscall_child.contract},
                                                           'customerno': {"label": "Customer No", "value": ucs_syscall_child.customer},
                                                           'siteno': {"label": "Site No", "value": ucs_syscall_child.site}}
        ...
        if [ucs_dict for ucs_dict in ucs_callhome_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_callhome_profiles(args={}):
    ...
    try:
        ...
        if [ucs_dict for ucs_dict in ucs_callhome_profiles_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_licenseinfo(args={}):
    ...
    try:
        ...
        for ucs_system_lic in handle.query_classid(class_id="LicenseFeature"):
            ucs_ini = {'fabric' : "", 'feature_type' : "", 'state' : "", 'default_qty' : "", 'total_qty' : "", 
                       'is_present' : "", 'total_used_qty' : "", 'peer_status' : "", 'time_left' : ""}
        ...
        if [ucs_dict for ucs_dict in ucs_licenseinfo_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_chassisinfo(args={}):
    ...
    try:
        ...
        for ucs_system_chassis in handle.query_classid(class_id="EquipmentChassis"):
            ucs_ini = {'chassis_id' : "", 'model' : "", 'serial_no' : "", 'state' : "", 'iom1_model':"", 'iom1_sn':"", 
                       'iom1_fw': "", 'iom2_model':"",  'iom2_sn':"", 'iom2_fw':""}
	    ...
	    if len(handle.query_children(in_mo=ucs_system_chassis)) != 0 :
                mo_chassis = handle.query_children(in_mo=ucs_system_chassis, class_id = "EquipmentIOCard")
                for ucs_system_chassis_child in mo_chassis:
                    if len(handle.query_children(in_mo=ucs_system_chassis_child)) != 0:
                        chassis_child = handle.query_children(in_mo=ucs_system_chassis_child, class_id = "MgmtController")
                            ...
                            if ucs_system_chassis_child.id == "1":
                                ...
                                for ucs_system_iomchild in chassis_child:
                                    if len(handle.query_children(in_mo=ucs_system_iomchild)) != 0:
                                        chassis_fw_child = handle.query_children(in_mo=ucs_system_iomchild, class_id = "FirmwareRunning")
        ...
        if [ucs_dict for ucs_dict in ucs_chassisinfo_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
    ...
    try:
        ...
        for server_type in server_types: 
            for ucs_sys_server in handle.query_classid(class_id=server_type):
                ucs_ini = {'location' : "", 'part_no' : "", 'model' : "", 'serial_no' : "", 'cpu_cores' : "", 
                           'memory' : "", 'mgmt_ip' : "", 'bios' : ""}
		...
		if len(handle.query_children(in_mo=ucs_sys_server)) != 0 :
                    mo_server_mgmt = handle.query_children(in_mo=ucs_sys_server, class_id = "MgmtController")
                    mo_server_bios = handle.query_children(in_mo=ucs_sys_server, class_id = "BiosUnit")
                    for ucs_sys_server_child in mo_server_mgmt:
                        if len(handle.query_children(in_mo=ucs_sys_server_child)) != 0:
                            server_child = handle.query_children(in_mo=ucs_sys_server_child, class_id = "VnicIpV4PooledAddr")
                    ...
                    for ucs_sys_server_child in mo_server_bios:
                        if ucs_sys_server_child.get_class_id() == "BiosUnit":
                            if len(handle.query_children(in_mo=ucs_sys_server_child)) != 0:
                                server_child = handle.query_children(in_mo=ucs_sys_server_child, class_id = "FirmwareRunning")
        ...
        if [ucs_dict for ucs_dict in ucs_serverinfo_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
        ...
        for server_type in server_types:
            for ucs_sys_server in handle.query_classid(class_id=server_type):
                ...
                if ucs_sys_server.assigned_to_dn != "":
                    ucs_serverinfo['service_profile_state'] = ucs_sys_server.assigned_to_dn.split("/")[1][3:] + \
                        "(" + ucs_sys_server.association + ")"
                if len(handle.query_children(in_mo=ucs_sys_server)) != 0:
                    mo_server_fw = handle.query_children(in_mo=ucs_sys_server, class_id = "FirmwareStatus")
                    mo_server_adaptor = handle.query_children(in_mo=ucs_sys_server, class_id = "AdaptorUnit")
                    mo_server_compboard = handle.query_children(in_mo=ucs_sys_server, class_id = "ComputeBoard")
                    ...
                    for ucs_sys_server_child in mo_server_adaptor:
                        if ucs_sys_server_child.get_class_id() == "AdaptorUnit":
                            ucs_serverinfo['adapters'].append(ucs_sys_server_child.id + ":" + ucs_sys_server_child.model + \
                                "," + ucs_sys_server_child.serial + "," + handle.query_dn(ucs_sys_server.dn + \
                                "/mgmt/fw-system").version)
                    for ucs_sys_server_child in mo_server_compboard:
                        if ucs_sys_server_child.get_class_id() == "ComputeBoard":
                            if len(handle.query_children(in_mo=ucs_sys_server_child)) != 0:
                                server_child = handle.query_children(in_mo=ucs_sys_server_child, class_id = "ProcessorUnit")
...
def get_ucs_system_faultinfo(args={}):
    ...
    try:
        ...
        if [ucs_dict for ucs_dict in ucs_faultinfo_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_syslog(args={}):
    ...
    try:
       ...
       if handle is not None:
           ...
           if [ucs_dict for ucs_dict in ucs_syslog_list if cmp(ucs_dict, ucs_ini) != 0] != []:
               #loginfo("Successfully fetched " + method)    
...
def get_ucsm_cluster(args={}):
    ...
    try:
        ...
        ucs_cluster_list.append(ucs_cluster.copy())
        if [ucs_dict.values for ucs_dict in ucs_cluster_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
            return PTK_OKAY, ucs_cluster_list, _("PDT_SUCCESS_MSG")
        loginfo("Unable to get " + method)
        return PTK_NOTEXIST, ucs_cluster_list, "Unable to get " + method
...
def get_ucsm_systemclass(args={}):
    ...
    try:
        ...
        if [ucs_dict for ucs_dict in ucs_sysclass_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_qospol(args={}):
    ...
    try:
        ...
        if handle is not None:
            ...
            if [ucs_dict for ucs_dict in ucs_qospol_list if cmp(ucs_dict, ucs_ini) != 0] != []:
                #loginfo("Successfully fetched " + method)
...
def get_ucs_system_nwctlpol(args={}):
    ...
    try:
        ...
        if [ucs_dict for ucs_dict in ucs_nwctlpol_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_vlaninfo(args={}):
	...
	lock.acquire()
        ...
        if [ucs_dict for ucs_dict in ucs_vlan_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_vsaninfo(args={}):
	...
	lock.acquire()
        ...
        if [ucs_dict for ucs_dict in ucs_vsan_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
	...
	lock.acquire()
        ...
        for fi_ethportchls in handle.query_classid(class_id="FabricEthLanPc"):
            ...
            for fi_fcoeportchls in handle.query_classid(class_id="FabricFcoeSanPc"):
                ...
                fi_portchl_list.append(fi_portchl.copy())
        if [ucs_dict for ucs_dict in fi_portchl_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_ethinterconnectinfo(args={}):
    ...
    try:
 	lock.acquire()
        ...
        if [ucs_dict for ucs_dict in fi_etherintercon_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_fcinterconnectinfo(args={}):
	...
	lock.acquire()
        ...
        if [ucs_dict for ucs_dict in fi_fcintercon_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_uuid_pool(args={}):
	...
	lock.acquire()
        ...
        if [ucs_dict for ucs_dict in fi_uuidpool_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_wwn_pool(args={}):
	...
	lock.acquire()
        ...
        if [ucs_dict for ucs_dict in fi_wwnpool_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_iqn_pool(args={}):
	...
	lock.acquire()
        ...
        if [ucs_dict for ucs_dict in fi_iqnpool_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_mac_pool(args={}):
    ...
    try:
 	lock.acquire()
        ...
        if [ucs_dict for ucs_dict in fi_macpool_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_ip_pool(args={}):
	...
	lock.acquire()
        ...
        if [ucs_dict for ucs_dict in fi_ippool_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_fault_policy(args={}):
	...
	lock.acquire()
        ...
        if [ucs_dict for ucs_dict in fi_fault_pol_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_backup_policy(args={}):
	...
	lock.acquire()
        ...
        if [ucs_dict for ucs_dict in fi_bkup_pol_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_ipmi_access_profiles(args={}):
	...
	lock.acquire()
        ...
        if [ucs_dict for ucs_dict in fi_ipmi_access_pol_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_local_disk_policy(args={}):
	...
	lock.acquire()
        ...
        if [ucs_dict for ucs_dict in fi_localdisk_pol_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_bios_policy(args={}):
	...
	lock.acquire()
        ...
        if [ucs_dict for ucs_dict in fi_bios_pol_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_maint_policy(args={}):
	...
	lock.acquire()
        ...
        if [ucs_dict for ucs_dict in fi_maint_pol_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_boot_policy(args={}):
    ...
    try:
        ...
        if [ucs_dict for ucs_dict in fi_boot_pol_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_lan_template(args={}):
	...
	lock.acquire()
            ...
            if len(mo_lantemp) != 0 :
                ...
                for vlans in handle.query_classid(class_id="FabricVlan"):
                    for vnic_ether_index in range(len(vnic_etherif)):
                        if (vlans.dn.split("/")[-1].split("net-")[1] == vnic_etherif[vnic_ether_index]):
        ...
        if [ucs_dict for ucs_dict in fi_lan_temp_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_san_template(args={}):
	...
	lock.acquire()
        ...
        if [ucs_dict for ucs_dict in fi_san_temp_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_host_fwpkg(args={}):
	...
	lock.acquire()
        ...
        if [ucs_dict for ucs_dict in fi_host_fw_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_server_pol_information(args={}):
	...
	lock.acquire()
        ...
        for fi_serverinfo in handle.query_classid(class_id="LsServer"):
            ...
            fi_server_info['maint_policy_name'] = handle.query_dn(fi_serverinfo.oper_maint_policy_name).name
            fi_server_info['maintenance_policy'] = handle.query_dn(fi_serverinfo.oper_maint_policy_name).uptime_disr
        ...
        if [ucs_dict for ucs_dict in fi_server_info_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
...
def get_ucs_system_server_information(args={}):
	...
	lock.acquire()
        ...
        for fi_serverinfo in handle.query_classid(class_id="LsServer"):
            ...
            fi_server_info['host_fw_pkg'] = handle.query_dn(fi_serverinfo.oper_host_fw_policy_name).name
            fi_server_info['kvm_ip_pool'] = handle.query_dn(fi_serverinfo.oper_kvm_mgmt_policy_name).name
            ...
            if len(mo_serverinterf) != 0:
                for serverinterf_child in mo_serverinterf:
                    if serverinterf_child.get_class_id() == "VnicEther":
                        ...
                        fi_server_interface['vnic_templ_name'].append(serverinterf_child.nw_templ_name)
                        fi_server_interface['mac'].append(serverinterf_child.addr)
                        fi_server_interface['mtu'].append(serverinterf_child.mtu)
                        fi_server_interface['vnic_adaptor_profile'].append(serverinterf_child.adaptor_profile_name)
            fi_server_interface_list.append(copy.deepcopy(fi_server_interface))
        if [ucs_dict for ucs_dict in fi_server_interface_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
	...
	lock.acquire()
        ...
        for fi_serverinterf in handle.query_classid(class_id="LsServer"):
            ...
            if len(mo_serverinterf) != 0:
                for serverinterf_child in mo_serverinterf:
                    if serverinterf_child.get_class_id() == "VnicEther":
                        fi_server_interface['vnic_qos_pol'].append(serverinterf_child.qos_policy_name)
                        fi_server_interface['nw_ctl_pol'].append(serverinterf_child.nw_ctrl_policy_name)
                        fi_server_interface['vnic_ident_pool'].append(serverinterf_child.ident_pool_name)
                        ...
                        if len(mo_adaptor_ether) != 0:
                            for adaptor_child in mo_adaptor_ether:
                                if adaptor_child.get_class_id() == "AdaptorEthVxLANProfile":
                                    fi_server_interface['virtual_ext_lan'].append(adaptor_child.admin_state)
                                if adaptor_child.get_class_id() == "AdaptorEthOffloadProfile":
                                    fi_server_interface['tcp_large_receive'].append(adaptor_child.large_receive)
                                if adaptor_child.get_class_id() == "AdaptorEthInterruptProfile":
                                    fi_server_interface['interrupt_timer'].append(adaptor_child.coalescing_time)
            fi_server_interface_list.append(copy.deepcopy(fi_server_interface))
        if [ucs_dict for ucs_dict in fi_server_interface_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
            return PTK_OKAY, fi_server_interface_list, _("PDT_SUCCESS_MSG")
            ...
            if len(mo_serverinterf) != 0:
                for serverinterf_child in mo_serverinterf:
                    ...
                    if serverinterf_child.get_class_id() == "VnicFc":
                        ...
                        fi_server_interface['vhba_templ_name'].append(serverinterf_child.nw_templ_name)
                        fi_server_interface['wwpn'].append(serverinterf_child.addr)
                        fi_server_interface['max_data'].append(serverinterf_child.max_data_field_size)
            fi_server_interface_list.append(copy.deepcopy(fi_server_interface))
        if [ucs_dict for ucs_dict in fi_server_interface_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
            return PTK_OKAY, fi_server_interface_list, _("PDT_SUCCESS_MSG")
	...
	lock.acquire()
        ...
        for fi_serverinterf in handle.query_classid(class_id="LsServer"):
            ...
            if len(mo_serverinterf) != 0:
                for serverinterf_child in mo_serverinterf:
                    if serverinterf_child.get_class_id() == "VnicFc":
                        fi_server_interface['vhba_adaptor_profile'].append(serverinterf_child.adaptor_profile_name)
                        fi_server_interface['vhba_qos_pol'].append(serverinterf_child.qos_policy_name)
                        fi_server_interface['vhba_ident_pool'].append(serverinterf_child.ident_pool_name)
                        fi_server_interface['vhba_order'].append(serverinterf_child.order)
                        adaptor_fc_dn = handle.query_dn(serverinterf_child.oper_adaptor_profile_name)
                        ...
                        if len(mo_adaptor_fc) != 0:
                            for adaptor_fc_child in mo_adaptor_fc:
                                if adaptor_fc_child.get_class_id() == "AdaptorFcPortProfile":
                                    fi_server_interface['io_throttle_cnt'].append(adaptor_fc_child.io_throttle_count)
                                    fi_server_interface['luns_per_target'].append(adaptor_fc_child.luns_per_target)
            fi_server_interface_list.append(copy.deepcopy(fi_server_interface))
        if [ucs_dict for ucs_dict in fi_server_interface_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
            return PTK_OKAY, fi_server_interface_list, _("PDT_SUCCESS_MSG")
            ...
            if len(mo_serverinterf) != 0:
                for serverinterf_child in mo_serverinterf:
                    ...
                    if serverinterf_child.get_class_id() == "VnicIScsi":
                        ...
                        fi_server_interface['iscsi_templ_name'].append(serverinterf_child.nw_templ_name)
                        fi_server_interface['iscsi_adaptor_profile'].append(serverinterf_child.adaptor_profile_name)
	...
	lock.acquire()
        ...
        for fi_service_profiles in handle.query_classid(class_id="LsServer"):
            ucs_ini = {'service_profile' : "", 'iscsi_primary' : "", 'iscsi_primary_initiator_address' : "", 
                       ...
                       'iscsi_primary_initiator_ident_pool' : "", 'initiator_primary_target_primary' : "", 
            ...
            if len(handle.query_children(in_mo=fi_service_profiles)) != 0:
                mo_servprof_child = handle.query_children(in_mo=fi_service_profiles, class_id = "VnicIScsiBootParams")
                    ...
                    if len(mo_iscsi_bootparams_child) != 0:
                        if service_profile_child.get_class_id() == "VnicIScsiBootParams":
                            for iscsi_bootparams_child in mo_iscsi_bootparams_child:
                                mo_iscsi_bootvnic_child = handle.query_children(in_mo=iscsi_bootparams_child)
                                if iscsi_bootparams_child.get_class_id() == "VnicIScsiBootVnic":
                                    ...
                                    if len(mo_iscsi_bootvnic_child) != 0 :
                                        for iscsi_bootvnic_child in mo_iscsi_bootvnic_child:
                                            mo_ini_targ_child  = handle.query_children(in_mo=iscsi_bootvnic_child)
        ...
        if [ucs_dict for ucs_dict in fi_iscsi_boot_params_list if cmp(ucs_dict, ucs_ini) != 0] != []:
            #loginfo("Successfully fetched " + method)
	...
	lock.acquire()
        ...
        for fi_service_profiles in handle.query_classid(class_id="LsServer"):
            ucs_ini = {'service_profile' : "", 'iscsi_secondary' : "", 'iscsi_secondary_initiator_address' : "", 
                       ...
                       'iscsi_secondary_initiator_ident_pool' : "", 'initiator_secondary_target_primary' : "", 
