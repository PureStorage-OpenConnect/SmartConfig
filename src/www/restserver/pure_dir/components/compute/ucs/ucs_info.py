...
class UCSInfo:
    ...
    def ucsmtopology(self):
        ...
        for fabric in fabrics:
            ...
            fi_connections.append(dict(name=fabric.rn, type="fi", id=fabric.id, leadership=leadership))
    		
        bladelist = self.handle.query_classid("ComputeBlade")
        blades = []
        for blade in bladelist:
	    #print blade
    	    blades.append(dict(name=blade.rn, type="blade", serial_no=blade.serial, model=blade.model, id=blade.slot_id, association=blade.association, discovery=blade.discovery, chassis_id=blade.chassis_id, operability=blade.operability, status=blade.oper_state, power=blade.oper_power))
        print blades
        ...
        for chassis in chassislist:
    	    ...
    	    fi_connections.append(dict(name=chassis.rn, type="chassis", serial_no=chassis.serial, acknowledged=chassis.admin_state, discovery=chassis.discovery, model=chassis.model, id=chassis.id, fi_conn=chassis.conn_path, operability=chassis.operability, status=chassis.oper_state, power="on" if chassis.power=="ok" else chassis.power, blades=chassis_blades))
        ...
        for server in servers:
            fi_connections.append(dict(name=server.rn, type="rack-server", serial_no=server.model, model=server.model, id=server.id, fi_conn=server.conn_path, operability=server.operability, status=server.oper_state, power=server.oper_power))
	...
	try:
            fi_connections = []
            fi_server_ports = [x for x in self.handle.query_classid('EtherPIo') if x.if_role=='server']
		...
		if sport.oper_state != "up" or sport.peer_dn is '':
		    print "Either server port " + local_interface + " is down or peer is not connected"
		    continue
                conn = {}
	        remote_device = sport.peer_dn.split('/')[1]
	        if remote_device.split('-')[0] == 'rack':
	            remote_port = re.search('adaptor-(.+)/ext-eth-(.+)', sport.peer_dn).groups()
	        ...
	        conn = dict(local_interface=local_interface, local_device=sport.dn.split('/')[1],
	    	            remote_interface=remote_interface, remote_device=remote_device, type='Server', 
		            state=sport.oper_state, speed=sport.oper_speed)
    ...
    def get_fc_port_detail(self, switch_id, fc_id):
        ...
        try:
            ...
            slot, port = [re.sub("\D", "", x) for x in fc_id.split('/')]
            fc_detail = [
                {
                    'interface_id': fc_id,
                    'speed': k.oper_speed,
		    ...
		    'pc':{'id':'Po'+re.compile('pc-(.+)/ep-slot').search(k.ep_dn).group(1), 'type':'FC PC'} if (k.is_port_channel_member and k.is_port_channel_member != 'no') else None,
    ...
    def get_ethernet_port_detail(self, switch_id, ether_id):
        ...
        try:
            ...
            slot, port = [re.sub("\D", "", x) for x in ether_id.split('/')]
            ether_detail = [{'interface_id': ether_id, 'speed': k.oper_speed, 'state': k.oper_state, 'type': 'Ethernet' if k.transport ==
                             'ether' else k.transport, 'pc':{'id':'Po'+re.compile('pc-(.+)/ep-slot').search(k.ep_dn).group(1), 'type':'Eth PC'} if (k.is_port_channel_member and k.is_port_channel_member != 'no') else None} for k in mo if k.slot_id == slot and k.port_id == port and k.switch_id == switch_id][0]
            return ether_detail
        except UcsException as e:
            loginfo(str(e))
            return None
