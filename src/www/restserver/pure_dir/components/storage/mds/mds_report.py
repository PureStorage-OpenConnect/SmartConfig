...
def mds_version(args={}):
    ...
    for switch in switch_name:
        helper = _mds_handler(switch)
        mds_init = {'serial_num': '', 'host_name': '', 'model': '', 'bios_version': '', 'kickstart_version': '',
                'sys_ver': '', 'hw_version': '', 'uptime': '', 'ipaddress': ''}
        mds_sys = copy.deepcopy(mds_init)
        if helper is not None:
            mds_show_hw = helper.mds_command('show hardware', ['serial_num','host_name', 'chassis_id', 'hw_ver', 'bios_ver_str', 
                                                               'rr_sys_ver', 'kickstart_ver_str','sys_ver_str'])
            mds_show_ut = helper.mds_command('show system uptime', ['sys_up_days', 'sys_up_hrs', 'sys_up_mins',
                                                                    'sys_up_secs'])
    ...
    if [mds_dict for mds_dict in mds_sys_info if (cmp(mds_dict, mds_init)) != 0] != []:
...
def mds_san_info(args={}):
    ...
    for switch in switch_name:
        helper = _mds_handler(switch)
        mds_init = {'domain': '', 'time': '', 'name': '', 'syslog': '', 'tacacs': '', 'scheduler': '', 'dns': '', 'ntp': '',
                    'npiv': ''}
        mds_sys = copy.deepcopy(mds_init)
        if helper is not None:
            try:
                ...
                mds_show_tacacs = helper.mds_command('show feature', ['ROW_cfcFeatureCtrl2Table'], cfcFeatureCtrlName2='tacacs')
                mds_show_scheduler = helper.mds_command('show feature', ['ROW_cfcFeatureCtrl2Table'], cfcFeatureCtrlName2='scheduler')
                mds_show_npiv = helper.mds_command('show feature', ['ROW_cfcFeatureCtrl2Table'], cfcFeatureCtrlName2='npiv')
                mds_name = helper.mds_command('show hardware', ['host_name'])
                mds_timezone = helper.mds_cmd_cli_err('show clock | grep UTC')
                mds_sys['time'] = (mds_timezone['simple_time']).split(' ')[1]
...
def mds_snmp_agent(args={}):
    ...
    for switch in switch_name:
        ...
        if helper != None:
            ...
            return PTK_RESOURCENOTAVAILABLE, mds_sys_info, "failed to get handler for mds Report Generation"
    if [mds_dict for mds_dict in mds_sys_info if (cmp(mds_sys, mds_init)) != 0] != []:
        #loginfo("Successfully fetched " + method)
        return PTK_OKAY, mds_sys_info, _("PDT_SUCCESS_MSG")
    else:
        loginfo("Unable to get " + method)
...
def mds_environment_info(args={}):
    ...
    switch_name = ['mds_switch_a', 'mds_switch_b']
    for switch in switch_name:
        mds_init = {'hostname': '', 'ps_model': [], 'ps_status': [], 'mds_mscs': [], 'fan_name': [], 'fan_model': [],
                   'fan_status': []}
        mds_sys = copy.deepcopy(mds_init)
        helper = _mds_handler(switch)
        if helper != None:    
            mds_show_hostname = helper.mds_command('show hardware', ['host_name'])
            mds_show_env = helper.mds_command('show environment ', ['ps_status', 'psmodel', 'tempmod', 
            ...
            mds_mod = zip(mds_show_env['tempmod'], mds_show_env['sensor'], mds_show_env['curtemp'], mds_show_env['alarmstatus'])
...
def mds_vsan_info(args={}):
    ...
    if helper is not None:
        mds_init = {'vsan_name': '', 'vsan_state': '', 'vsan_interop_mode': '', 'vsan_load_balancing': '', 'vsan_operational_state': ''}
        mds_sys = copy.deepcopy(mds_init)
...
def mds_interfaces_fc(args={}):
    ...
    if helper is not None:
        mds_sys = copy.deepcopy(mds_init)
        mds_show_module = helper.mds_command('show interface brief', ['fcot_info', 'ROW_interface_brief_fc', 
                                                                      'status', 'port_rate_mode', 'oper_speed', 'interface_fc'])
        mds_descrip = helper.mds_command('show interface description', ['ROW_interface'])
        mds_port_lic = helper.mds_port_license()

        tmp_dict['description'] = [v for i in mds_descrip['ROW_interface'] for v in i['sfp_desc']][0:48]
        tmp_dict['status'] = [j['status'] for i in mds_show_module['ROW_interface_brief_fc'] for j in i]
        tmp_dict['port_rate_mode'] = [j['port_rate_mode'] for i in mds_show_module['ROW_interface_brief_fc'] for j in i]
        tmp_dict['oper_speed'] = [j['oper_speed'] for i in mds_show_module['ROW_interface_brief_fc'] for j in i]
        tmp_dict['fcot_info'] = [j['fcot_info'] for i in mds_show_module['ROW_interface_brief_fc'] for j in i]
        tmp_dict['interface'] = ["".join([v for k,v in i.items() if k == "interface"]) for i in mds_port_lic]
        tmp_dict['license'] = ["".join([v for k,v in i.items() if k == "licns"]) for i in mds_port_lic]

        mds_smsml = izip_longest(tmp_dict['status'], tmp_dict['port_rate_mode'],
...
def mds_zoneset(args={}):
    ...
    if helper is not None:
        mds_sys = copy.deepcopy(mds_init)
        mds_zoneset_name = helper.mds_command('show zoneset brief', ['zoneset_vsan_id', 'zoneset_name'])
