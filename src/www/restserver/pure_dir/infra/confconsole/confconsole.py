...
PLUGIN_PATH = os.path.join(os.path.dirname(
    os.path.realpath(__file__)), 'plugins.d')
...
class TurnkeyConsole:
    ...
    def __init__(self, pluginManager, eventManager, advanced_enabled=True):
        title = "FlashStack SmartConfig Console"
        self.width = 60
        self.height = 20

        self.console = Console(title, self.width, self.height)
        self.appname = "GNU/Linux %s" % netinfo.get_hostname().upper()
    ...
    def _get_advmenu(self):
        ...
        items.append(("Reset", "Reset the Application"))
    ...
    def usage(self):
        ...
        try:
            ...
            retcode = self.console.msgbox("Tool Configuration",
    ...
    def _ifconf_dhcpserver(self):
        def _validate(start, end, netmask, gateway, nameservers):
            errors = []

            if not start:
                errors.append("No DHCP Start Range provided")
            elif not ipaddr.is_legal_ip(start):
                errors.append(
                    "Invalid DHCP Start Range IP address: %s" % start)

            if not end:
                errors.append("No DHCP End Range provided")
            elif not ipaddr.is_legal_ip(end):
                errors.append("Invalid DHCP End Range IP address: %s" % end)
        ...
        input = [start, end, netmask, gateway]
        ...
        while 1:
            fields = [
                ("DHCP Start Range", input[0], field_width, field_limit),
                ("DHCP End Range", input[1], field_width, field_limit),
                ("Netmask", input[2], field_width, field_limit),
                ("Default Gateway", input[3], field_width, field_limit),
            ]

            for i in range(len(input[4:])):
                fields.append(
                    ("Name Server", input[4 + i], field_width, field_limit))

            text = "DHCP Server configuration"
            retcode, input = self.console.form(
                "DHCP Server settings", text, fields)
            ...
            start, end, netmask, gateway = input[:4]
            nameservers = input[4:]
            for i in range(nameservers.count('')):
                nameservers.remove('')
            err = _validate(start, end, netmask, gateway, nameservers)
            if err:
                err = "\n".join(err)
            else:
                err = ifutil.set_dhcpserver(start, end, netmask,
                                            gateway, nameservers)
                if not err:
    ...
    def _ifconf_staticip(self):
        def _validate(addr, netmask, gateway):
		...
		return ["Nameserver "]
            for nameserver in nameservers:
                if nameserver and not ipaddr.is_legal_ip(nameserver):
                    errors.append("Invalid nameserver: %s" % nameserver)
                if nameserver.split(".")[0] == "127":
                    return ["Loopback address range not allowed to set as DNS"]
        ...
        while 1:
            ...
            if not input[0] and not input[1] and not input[2]:
            ...
            err = _validate(addr, netmask, gateway)
            if err:
                err = "\n".join(err)
            else:
    ...
    def _reset(self, text, opt):
        if self.console.yesno(text) == self.OK:
            self.running = False
            for name in Applications:
                cmd = "curl http://127.0.0.1/%s/%sReset" % (name, name.upper())
                executil.system(cmd)
        return "advanced"

    def _adv_reset(self):
        return self._reset("Reset the Application?", "-c")
