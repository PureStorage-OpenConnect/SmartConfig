...
class SimpleMysql:
    ...
    def __init__(self, **kwargs):
        ...
        self.conf['db'] = kwargs.get("db", "puredir")
        self.conf['user'] = kwargs.get("user", "root")
        self.conf['passwd'] = kwargs.get("passwd", "root")
    ...
    def getOne(self, table=None, fields='*', where=None, order=None, like=None, limit=(0, 1), month=0, year=0):
        """Get a single result
        ...
        cur = self._select(table, fields, where, order,
                           limit, like, month, year)
        result = cur.fetchone()

        return result

    def getAll(self, table=None, fields='*', where=None, order=None, limit=None, like=None, month=0, year=0):
        """Get all results
    ...
    def leftJoin(self, tables=(), fields=(), join_fields=(), where=None, order=None, limit=None, like=None, month=0, year=0):
        ...
        cur = self._select_join(
            tables, fields, join_fields, where, order, limit, like, month, year)
        result = cur.fetchall()
    ...
    def _select(self, table=None, fields=(), where=None, order=None, limit=None, like=None, month=0, year=0):
        """Run a select query"""
        ...
        if month != 0 and year != 0:
            sql += "WHERE Month(Date)=%d and Year(Date)=%d " % (month, year)
        ...
        if like:
            likes = "%" + like[1] + "%"
            sql += "WHERE %s like \"%s\"" % (like[0], likes)
