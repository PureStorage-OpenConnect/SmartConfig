    ...
    def rollback(self, inputs, outputs, logfile):
        ...
        if cred:
            obj = NEXUSTasks(
                    ipaddress=cred['ipaddress'], username=cred['username'], password=cred['password'])
            if obj:
                res = obj.nexusRemoveGlobalConfigurations(inputs, logfile, "n5k")
            else:
                customlogs("Failed to login to NEXUS switch", logfile)
                loginfo("Failed to login to NEXUS switch")
                res.setResult(False, PTK_INTERNALERROR,
...
class NEXUS5kSetGlobalConfigurationsInputs:
    ...
    route = Textbox(validation_criteria='ip', hidden='', isbasic='True', helptext='Route address', dt_type="string",
                    ...
                    static_values="", svalue="0.0.0.0/0", mapval="", mandatory="1", order=2, recommended="1")
    gateway = Textbox(validation_criteria='ip', hidden='', isbasic='True', helptext='Gateway address', dt_type="string",
                      static="False", api="", name="gateway", label="Gateway",
                      static_values="", svalue="192.168.10.28", mapval="", mandatory="1", order=3, recommended="1")
    ntp = Textbox(validation_criteria='ip', hidden='', isbasic='True', helptext='NTP address', dt_type="string",
                  static="False", api="", name="ntp", label="NTP",
                  static_values="", svalue="192.168.10.29", mapval="", mandatory="1", order=4, recommended="1")
