...
g_flash_stack_types = [
    {'label': 'FA//FI ', 'value': 'fs-mini', 'tag': 'FC', 'enabled': False, 'req_hardwares': {'UCSM': 2, 'PURE': 1}},
    {'label': 'FA//FI ', 'value': 'fa-fi-iscsi', 'tag': 'iSCSI', 'enabled': False, 'req_hardwares': {'UCSM': 2, 'PURE': 1}},
    {'label': 'FA//MDS//Nexus 9K//FI',
        'value': 'fa-n9k-fi-mds-fc', 'tag': 'FC', 'enabled': True, 'req_hardwares': {'UCSM': 2, 'Nexus 9k': 2, 'MDS': 2, 'PURE': 1}},
    {'label': 'FA//MDS//FI ', 'value': 'fa-mds-fi-fc', 'tag': 'FC', 'enabled': False, 'req_hardwares': {'UCSM': 2, 'Nexus 9k': 2, 'MDS': 2, 'PURE': 1}},
...
def workflow_info_api(wid):
    ...
    """
    #wfs = glob.glob(get_workflow_files_pattern())
    wfs = get_all_workflows()
    obj = result()
    flag = False
    for wf in wfs:
...
def check_job_with_wid_exists(wname):
    ...
    for job in jobs:
        try:
            ...
            loginfo(str(e))
            obj.setResult(None, PTK_INTERNALERROR, _("PDT_UNEXPECTED_INTERNAL_ERR_MSG"))
            return obj
...
def import_workflow_api(uploadfile):
    ...
    if os.path.splitext(uploadfile.filename)[1] == ".zip":
        ...
        res.setResult(False, PTK_INTERNALERROR, _("PDT_UNSUPPORTED_FILE_TYPE_ERR_MSG"))
    return res
