...
metadata = dict(
    task_id="NEXUS9kConfigureVirtualPortChannelsToFB",
    task_name="Configure Virtual Port Channels To FlashBlade",
    task_desc="Configure Virtual Port Channels To FlashBlade for Nexus switch",
    task_type="NEXUS"
)


class NEXUS9kConfigureVirtualPortChannelsToFB:
    def __init__(self):
        pass

    def execute(self, taskinfo, logfile):
        res = result()
        loginfo("NEXUS Configure Virtual Port Channels To FlashBlade")
        ...
        if cred:
            ...
            if obj:
                res = obj.nexusConfigureVirtualPortChannelsToFB(
    ...
    def rollback(self, inputs, outputs, logfile):
        res = result()
        loginfo("NEXUS Configure Virtual Port Channels To FlashBlade rollback")
        cred = get_device_credentials(
        ...
        if cred:
            obj = NEXUSTasks(
                ipaddress=cred['ipaddress'], username=cred['username'], password=cred['password'])
            if obj:
                res = obj.nexusUnconfigureVirtualPortChannelsToFB(
                    inputs, logfile, "n9k")
            else:
                customlogs("Failed to login to NEXUS switch", logfile)
                loginfo("Failed to login to NEXUS switch")
...
class NEXUS9kConfigureVirtualPortChannelsToFBInputs:
    nexus_id = Dropdown(
        hidden='True',
        isbasic='',
        helptext='',
...
class NEXUS9kConfigureVirtualPortChannelsToUCSFBOutputs:
