...
class FAConnectVolumeToFCHost:
    ...
    def execute(self, inputs, logfile):
        """
        :param inputs: task input for FAConnectVolumeToFCHost like hostname, volumename
        :type inputs: dict
        :param logfile: for printing logs
        :type logfile: function
        :returns: A dictionary describing the list of host mapped with respectice volume
        :rtype: dict
    ...
    def rollback(self, inputs, outputs, logfile):
        """
        :param inputs: task input for FAConnectVolumeToFCHost
        :type inputs: dict
        :param outputs: task output for FAConnectVolumeToFCHost
        :type outputs: dict
        :param logfile: for printing logs
        :type logfile: function
    ...
    def prepare(self, jobid, texecid, inputs):
        ...
        for pre in range(1, blade_len + 1):
            if pre < 10:
                ...
                vol_prefix = 'VM-Vol-FC-' + str(pre).zfill(3)
