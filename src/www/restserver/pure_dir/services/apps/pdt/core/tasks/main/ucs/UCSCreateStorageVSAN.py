...
metadata = dict(
    task_id="UCSCreateStorageVSAN",
    task_name="Create Storage vSAN in UCS",
    task_desc="Create Storage vSAN for UCS direct connectivity to FlashArray",
    task_type="UCSM"
)


class UCSCreateStorageVSAN:
    def __init__(self):
        pass
    ...
    def execute(self, taskinfo, fp):
        ...
        obj = res.getResult()
        res = obj.ucsCreateStorageVSAN(taskinfo['inputs'], fp)

        obj.release_ucs_handle()
        return parseTaskResult(res)

    def rollback(self, inputs, outputs, logfile):
        loginfo("UCS rollback storage VSAN")
        res = get_ucs_handle(inputs['fabric_id'])
        if res.getStatus() != PTK_OKAY:
            return parseTaskResult(res)
        obj = res.getResult()
        res = obj.ucsDeleteStorageVSAN(inputs, logfile)
        obj.release_ucs_handle()
        return res
