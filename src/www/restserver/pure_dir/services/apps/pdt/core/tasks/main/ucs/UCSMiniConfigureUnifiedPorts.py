...
metadata = dict(
    ...
    task_name="Configure Unified Ports in UCS Mini",
    task_desc="Configure Unified Ports in UCS Mini",
    task_type="UCSM"
)


class UCSMiniConfigureUnifiedPorts:
    def __init__(self):
        pass
    ...
    def get_ucs_ports(self, keys):
        ...
        slot_info = {"min_range": str(1), "max_range": str(
            4), "min_fixed": True, "max_fixed": False, "min_interval": "1"}
        details['label'] = ""
        details['id'] = ""
        details['selected'] = str(1) + "-" + str(2)
...
class UCSMiniConfigureUnifiedPortsInputs:
    fabric_id = Dropdown(
        hidden='True',
        isbasic='True',
        helptext='',
...
class UCSMiniConfigureUnifiedPortsOutputs:
    status = Output(dt_type="integer", name="status", tvalue="SUCCESS")
    fabric_id = Output(dt_type="string", name="fabric_id", tvalue="A")
    no_of_ports = Output(dt_type="integer", name="no_of_ports", tvalue="2")
