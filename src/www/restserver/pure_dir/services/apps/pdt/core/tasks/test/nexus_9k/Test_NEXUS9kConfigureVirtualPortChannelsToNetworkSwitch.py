...
class Test_NEXUS9kConfigureVirtualPortChannelsToNetworkSwitch:
    def __init__(self):
        pass

    def execute(self, taskinfo, logfile):
        loginfo("Test NEXUS Configure Virtual Port Channels To Network Switch")

        res = getMappedOutputs(taskinfo['jid'], taskinfo['texecid'])
        return res.getResult()

    def rollback(self, inputs, outputs, logfile):
        print("NEXUS Configure Virtual Port Channels To Network Switch")
        res = result()
