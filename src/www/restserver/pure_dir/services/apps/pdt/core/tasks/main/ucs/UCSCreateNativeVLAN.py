...
metadata = dict(
    task_id="UCSCreateNativeVLAN",
    task_name="Create Nstive VLAN",
    task_desc="Create Native VLAN in UCS",
    task_type="UCSM"
)


class UCSCreateNativeVLAN:
    def __init__(self):
        pass
    ...
    def execute(self, taskinfo, logfile):
        ...
        res = obj.ucsCreateNativeVLAN(taskinfo['inputs'], logfile)

        obj.release_ucs_handle()
        return parseTaskResult(res)

    def rollback(self, inputs, outputs, logfile):
        loginfo("UCS Native VLAN creation rollback")
        res = get_ucs_handle(inputs['fabric_id'])
        if res.getStatus() != PTK_OKAY:
            return res
        obj = res.getResult()

        res = obj.ucsDeleteNativeVLAN(
...
class UCSCreateNativeVLANInputs:
    fabric_id = Dropdown(
        hidden='True',
        isbasic='True',
        helptext='',
...
class UCSCreateNativeVLANOutputs:
