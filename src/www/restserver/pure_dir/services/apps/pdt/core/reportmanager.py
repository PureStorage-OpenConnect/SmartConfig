...
class SCReport:
    ...
    def report(self, stacktype):
        ...
        try:
            ...
            for section in root:
                ...
                if 'unsupported_stacktypes' in section.attrib and section.attrib['unsupported_stacktypes'] != '':
                    if stacktype in section.attrib['unsupported_stacktypes'].split('|'):
                        continue
    ...
    def initiate_sheet_thread(self, report_api_res):
        ...
        for page in pages:
            ...
            if page == 'Topology Data':
                ...
                api_dict[page] = [
                    report_res for report_res in report_api_res if report_res['belongsTo'] == page]
        for page in pages:
            thread_page[str(page)] = threading.Thread(
                target=self.call_method_by_page, args=(api_dict[page], api_results))
    ...
    def write_sheets_excel(
        ...
        for report_res in report_api_res:
            ...
            try:
                ...
                self.generate_title_excel(
                    cell_range, start_col, till_col, title, sheet, cell_border)

                # Table Header section
                (cell_range, start_col, till_col) = self.generate_cell_range(
                    length, sheet._current_row, start_col_alpha)
                self.generate_header_excel(
                    cell_range, start_col, till_col, headers, sheet, cell_border)

                # Table Data Section
                self.generate_data_excel(
                    length,
                    keys,
                    report_info_output['list'],
                    sheet,
                    cell_border,
                    start_col_alpha,
                    merge,
    ...
    def generate_generic_info(
        ...
        try:
            ...
            if os.path.exists(logo_path):
                width = 200
                height = 50
                if ((system_info_res.get('report_logo') is not None) and
                    ...
                    self.add_image_sheet(sheet, 'G2', path_pref +
                                         system_info_res.get('report_logo'), width, height)
                else:
            ...
            if "fb" in stacktype:
                stack_label += " [NFS]"
            elif stacktype.split('-')[-1].lower() == 'fc':
                stack_label += " [FC]"
            elif stacktype.split('-')[-1].lower() == 'iscsi':
                stack_label += " [iSCSI]"
            ...
            # Table Title Section
            length = len(list(general_list[0].values()))
    ...
    def generate_topology_info(
        ...
        try:
            ...
            merge = True
            merge_cols = 1
            for components in list(component_dict.values()):
                for component in components:
                    try:
                        fs_result = []
                        header_topo = {}
                        fs_result = get_fs_connections(
                            fs_con_data, component, device_name_dict[component])
                        header_topo = fs_result[0]
                        fs_result.pop(0)

                        # Table Title Section
                        length = len(list(header_topo.values()))
    ...
    def generate_title_excel(self, cell_range, start_col, till_col, title, sheet, cell_border):
        for title_count in range(len(cell_range)):
            ...
            title_cell = sheet.cell(row=sheet._current_row,
                                    column=(ord(start_col[0]) - ord('A')) + 1)
            title_cell.value = title
    ...
    def generate_header_excel(self, cell_range, start_col, till_col, headers, sheet, cell_border):
        for header_count in range(len(cell_range)):
            ...
            sheet[cell_range[header_count]].alignment = Alignment(
    ...
    def generate_data_excel(
        ...
        if merge:
            for col_count in range(0, merge_cols):
                sheet.merge_cells(chr(ord(start_col_alpha) + col_count) + start_row +
                                  ':' + chr(ord(start_col_alpha) + col_count) + end_row)

        for column_cells in sheet.columns:
            unmerged_cells = list(
                filter(
                    lambda cell_to_check: cell_to_check.coordinate not in sheet.merged_cells,
                    column_cells))
            length_list = []
