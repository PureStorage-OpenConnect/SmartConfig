...
metadata = dict(
    task_id="NEXUS9kConfigureVirtualPortChannelsToUCSFB",
...
class NEXUS9kConfigureVirtualPortChannelsToUCSFB:
    def __init__(self):
        pass
        ...
        if cred:
            ...
            if obj:
                res = obj.nexusConfigureVirtualPortChannelsToUCS(
                    taskinfo['inputs'], logfile, "n9k", flashBlade=True)
            else:
        ...
        if cred:
            ...
            if obj:
                res = obj.nexusUnconfigureVirtualPortChannelsToUCS(
                    inputs, logfile, "n9k", flashBlade=True)
            else:
                customlogs("Failed to login to NEXUS switch", logfile)
                loginfo("Failed to login to NEXUS switch")
                res.setResult(False, PTK_INTERNALERROR,
...
class NEXUS9kConfigureVirtualPortChannelsToUCSFBInputs:
    nexus_id = Dropdown(
        hidden='True',
        isbasic='',
        helptext='',
