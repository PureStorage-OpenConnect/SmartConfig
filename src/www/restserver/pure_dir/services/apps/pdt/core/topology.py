...
def get_fi_server_interfaces(fi_object, fi_tags):
    ...
    [conn.update({'local_device':[fi['name'] for fi in fi_tags if fi['tag']==conn['local_device'].split('-')[1]][0]}) for conn in server_conn]
    server_ifaces = {}
    for fi in fi_tags:
        server_ifaces[fi['name']] = [conn for conn in server_conn if conn['local_device']==fi['name']]
    server_ifaces = {fi:[iface if iface.pop('local_device',None) else None for iface in ifaces] for fi, ifaces in server_ifaces.items()}
...
def get_nexus_fa_connectivity(nx_obj, fa_obj):
    ...
    for mac_entry in nx_mac_table:
        for hwaddr_entry in fa_nw_ifaces:
            if mac_entry['disp_mac_addr'].replace('.', '').lower() == hwaddr_entry['hwaddr'].replace(':', '').lower():
	        #print  mac_entry['disp_port'], mac_entry['disp_mac_addr'].replace('.', '').lower(), hwaddr_entry['hwaddr'].replace(':', '').lower()
...
def get_fa_nexus_connectivity(fa_obj, nx_obj):
    ...
    for hwaddr_entry in fa_nw_ifaces:
        for mac_entry in nx_mac_table:
            if hwaddr_entry['hwaddr'].replace(':', '').lower() ==  mac_entry['disp_mac_addr'].replace('.', '').lower():
		intf_details = [iface for iface in fa_obj.get_hardware_conf() if iface['name']==hwaddr_entry['name']][0]
		#intf_details = nx_obj.get_interface_details(mac_entry['disp_port'])
		fa_nx_entry = dict(remote_interface=mac_entry['disp_port'],
				   local_interface=hwaddr_entry['name'],
				   speed=str(intf_details['speed']/(1000*1000*1000)) + "Gb/s",
...
def get_ucs_dc_fc_fa_connectivity(fi_dict, fa_obj, fi_tags):
    ...
    for fi in fi_tags:
        fc_fa_list[fi['name']] = []
        fi_flogi_db[fi['name']] = ucs_info_netmiko.get_flogi_sessions(fi_dict['vipaddress'], fi_dict['username'], decrypt(fi_dict['password']), fi['tag'])
	for flogi in fi_flogi_db[fi['name']]:
	    for port in fa_ports:
	        if flogi['pwwn'].lower().replace(':', '') == port['wwn'].lower():
		    intf_details = fi_dict['object'].get_fc_port_detail(fi['tag'], flogi['iface_id'])
		    fc_fa_entry = dict(local_interface=flogi['iface_id'],
		    		                       type=intf_details['type'],
				                       speed=intf_details['speed'],
				                       state=intf_details['state'],
...
def get_ucs_dc_iscsi_fa_connectivity(fi_dict, fa_obj, fi_tags):
    ...
    for fi in fi_tags:
        fi_fa_list[fi['name']] = []
        fi_iscsi_intf[fi['name']] = ucs_info_netmiko.get_mac_address_table(fi_dict['vipaddress'], fi_dict['username'], decrypt(fi_dict['password']), fi['tag'])
	for mac_entry in fi_iscsi_intf[fi['name']]:
	    for hwaddr_entry in fa_nw_ifaces:
	        if mac_entry['mac'].replace('.', '').lower() == hwaddr_entry['hwaddr'].replace(':', '').lower():
		    intf_details = [iface for iface in fa_obj.get_hardware_conf() if iface['name']==hwaddr_entry['name']][0]
		    fi_fa_entry = dict(local_interface=mac_entry['iface_id'],
				                        remote_interface=hwaddr_entry['name'],
				                        speed=str(intf_details['speed']/(1000*1000*1000)) + "Gb/s",
				                        state='up' if intf_details['status']=='ok' else intf_details['status'],
				                        type='Ethernet')
    ...
    for port in fa_ports:
        for fi, flogi_list in fi_flogi_db.items():
	        for flogi in flogi_list:
		        ...
		        intf_details = [iface for iface in fa_obj.get_hardware_conf() if iface['name']==port['name']][0]
	    	        fa_fc_entry = dict(remote_interface=flogi['iface_id'], remote_device=fi,
				                            type='FC',
				                            speed=str(intf_details['speed']/(1000*1000*1000)) + "Gb/s",
				                            state='up' if intf_details['status']=='ok' else intf_details['status'],
    ...
    for hwaddr_entry in fa_nw_ifaces:
        for fi, iscsi_ifaces in fi_iscsi_intf.items():
	        for iscsi_if in iscsi_ifaces:
 		        if hwaddr_entry['hwaddr'].replace(':', '').lower() == iscsi_if['mac'].replace('.', '').lower():
		            intf_details = [iface for iface in fa_obj.get_hardware_conf() if iface['name']==hwaddr_entry['name']][0]
		            fa_fi_entry = dict(remote_interface=iscsi_if['iface_id'], remote_device=fi,
				                       type='Ethernet',
				                       speed=str(intf_details['speed']/(1000*1000*1000)) + "Gb/s",
				                       state='up' if intf_details['status']=='ok' else intf_details['status'],
				                       local_interface=hwaddr_entry['name'])
...
def get_fa_fc_connectivity(fa_obj, fc_switch_obj):
    ...
    for port in fa_ports:
	    ...
	    if port['wwn'].lower() == flogi['pwwn'].lower().replace(':', ''):
		intf_details = [iface for iface in fa_obj.get_hardware_conf() if iface['name']==port['name']][0]
		fa_fc_entry = dict(remote_interface=flogi['iface_id'],
				    type='FC',
				    speed=str(intf_details['speed']/(1000*1000*1000)) + "Gb/s",
				    state='up' if intf_details['status']=='ok' else intf_details['status'],
				    local_interface=port['name'])
...
def get_san_ucs_connectivity(switch_type, san_obj, fi_list):
    ...
    fi_obj = [fi for fi in fi_list if fi['leadership'] == 'primary'][0]['object']
    ucs_san_list = []
    for fi in fi_list: 
	san_neighbors = fi_obj.get_san_neighbors(fi['tag'])
    ...
    san_pc_intf = [intf_san for intf_san in san_flogi_db for intf_ucs in ucs_san_list if intf_san['pwwn']==intf_ucs['my_pwwn']]
    ...
    ucs_pc_intf = [intf_ucs for intf_ucs in ucs_san_list for intf_san in san_flogi_db if intf_san['pwwn']==intf_ucs['my_pwwn']]
    ucs_pc =  re.compile('san-port-channel (.+)').search(ucs_pc_intf[0]['local_interface']).group(1)
    ucs_det_for_san_intf = [intf_ucs for intf_san in san_flogi_db for intf_ucs in ucs_san_list if intf_san['pwwn']==intf_ucs['my_pwwn']][0]
    ucs_fc_list = fi_obj.get_san_pc_members(ucs_det_for_san_intf['tag'], ucs_pc)
...
def get_ucs_san_connectivity(fi_obj, fi_tags):
    ucs_san_list = {}
    for fi in fi_tags:
        ucs_san_neighbors_list = fi_obj.get_san_neighbors(fi['tag'])
        ucs_san_list[fi['name']] = get_ucs_san_pc_interfaces(fi_obj, ucs_san_neighbors_list, fi['tag'])
    return ucs_san_list

def get_ucs_san_pc_interfaces(fi_obj, ucs_san_list, fi_tag):
    ucs_san_pc_interfaces = []
    for neighbor in ucs_san_list:
        if 'san-port-channel' in neighbor['local_interface']:
	    ucs_san_pc = re.compile('san-port-channel (.+)').search(neighbor['local_interface']).group(1)
	    ...
	    fc_interface = [flogi for flogi in fc_flogi_list if flogi['pwwn']==neighbor['my_pwwn'] and flogi['nwwn']==neighbor.get('my_nwwn', neighbor.get('fabric_pwwn', None))]
	    if fc_interface:
	        fc_san_pc =  re.compile('port-channel(.+)|San-po(.+)').search(fc_interface[0]['iface_id']).group(1)
...
def get_connection(speed, media_type):
    speed_ops = re.compile('^(\d+)( ?)(\w{2})(.*)').search(speed.lower())
...
def topology_n9k(switch_name, switch_object, switch_type, stacktype, fs_objs, queue):
	...
	if stacktype['tag'] == 'iSCSI':
        ...
        queue.put({switch_name:cdp_nbors + fa_neighbors})
    except Exception as e:
	loginfo(str(e))
        queue.put({switch_name:cdp_nbors + fa_neighbors})
    return
...
def topology_fi(switch_name, switch_object, switch_type, stacktype, fs_objs, queue):
    ...
    fi_tags = [{'tag':hw['tag'], 'name':hw['name']} for hw in fs_objs if hw['device_type'] == 'UCSM']
    try:
        ...
        if stacktype['tag'] == 'FC':
            ucs_san_neighbors = get_ucs_san_connectivity(switch_object, fi_tags)
            if 'ucsmini' in stacktype['value']:
	            fa = [hw for hw in fs_objs if hw['device_type']=='PURE'][0]
	            fi_dict = [hw for hw in fs_objs if hw['device_type']=='UCSM' and hw['leadership']=='primary'][0]
		    ucs_fa_neighbors = get_ucs_dc_fc_fa_connectivity(fi_dict, fa['object'], fi_tags)
    	            {fi:[intf.update({'remote_device':fa['name']}) for intf in intf_list] for fi, intf_list in ucs_fa_neighbors.items()}
		    ...
		    ucs_fa_neighbors = get_ucs_dc_iscsi_fa_connectivity(fi_dict, fa['object'], fi_tags)
    	            {fi:[intf.update({'remote_device':fa['name']}) for intf in intf_list] for fi, intf_list in ucs_fa_neighbors.items()}
        for fi in fi_tags:
            fi_neighbors[fi['name']] = ucs_lan_neighbors.get(fi['name'], []) + ucs_san_neighbors.get(fi['name'], []) + ucs_server_interfaces.get(fi['name'], []) +  ucs_fa_neighbors.get(fi['name'], [])
...
def topology_fa(switch_name, switch_object, switch_type, stacktype, fs_objs, queue):
    ...
    try:
	    ...
	    if 'ucsmini' in stacktype['value']:
    		fi_tags = [{'tag':hw['tag'], 'name':hw['name']} for hw in fs_objs if hw['device_type'] == 'UCSM']
	        fi_dict = [hw for hw in fs_objs if hw['device_type']=='UCSM' and hw['leadership']=='primary'][0]
		fa_neighbors = fa_neighbors + get_fa_ucs_dc_fc_connectivity(switch_object, fi_dict, fi_tags)
        elif stacktype['tag'] == 'iSCSI':
	    ...
	    if 'ucsmini' in stacktype['value']:
    		fi_tags = [{'tag':hw['tag'], 'name':hw['name']} for hw in fs_objs if hw['device_type'] == 'UCSM']
	        fi_dict = [hw for hw in fs_objs if hw['device_type']=='UCSM' and hw['leadership']=='primary'][0]
		fa_neighbors = fa_neighbors + get_fa_ucs_dc_iscsi_connectivity(switch_object, fi_dict, fi_tags)
...
def construct_topology(stacktype, fs_objs):
    ...
    for component in fs_objs:
        if component['device_type'] == 'Nexus 9k':
            t=Thread(name=component['name'], target=topology_n9k, args=(component['name'], component['object'], component['device_type'], stacktype, fs_objs, q))
            t.start()
        elif component['device_type'] == 'Nexus 5k':
            t=Thread(name=component['name'], target=topology_n5k, args=(component['name'], component['object'], component['device_type'], stacktype, fs_objs, q))
            t.start()
        elif component['device_type'] == 'MDS':
            t=Thread(name=component['name'], target=topology_mds, args=(component['name'], component['object'], component['device_type'], stacktype, fs_objs, q))
            t.start()
        elif component['device_type'] == 'UCSM' and component['leadership'] == 'primary':
            t=Thread(name=component['name'], target=topology_fi, args=(component['name'], component['object'], component['device_type'], stacktype, fs_objs, q))
            t.start()
        elif component['device_type'] == 'PURE':
            t=Thread(name=component['name'], target=topology_fa, args=(component['name'], component['object'], component['device_type'], stacktype, fs_objs, q))
            t.start()
	else:
	    continue
        threads.append(t)
    ...
    [x.update({'connection':get_connection(x[y], x['type'])}) for hw,conn in fs_connections.items() for x in conn for y,z in x.items() if y=='speed' and x[y]!='indeterminate']
...
def fs_connectivity():
	...
	fs_objs = get_stack_obj()
        ...
        components = {hw_type: [{k:v for k,v in comp.items() if k in ['name', 'ipaddress', 'mac', 'serial_no', 'leadership', 'model', 'vipaddress']} for comp in fs_objs if hw_type in comp['device_type']] for hw_type in hw_types}
        fi_object = [component for component in fs_objs if component['device_type'] == 'UCSM' and component['leadership'] == 'primary'][0]['object']
	fi_server_components = fi_object.ucsmtopology()
        fi_server_types = ['chassis', 'rack-server']
	fi_server_components = {fi_server_type.upper():[{k:v for k,v in comp.items()} for comp in fi_server_components if fi_server_type in comp['type']] for fi_server_type in fi_server_types}
	...
	connections = {hw:[conn for conn in conns if conn['remote_device'] in fs_comp_names] for hw, conns in connections.items()}

        fs_connectivity = {'stacktype':stack['value'], 'components':components, 'connections':connections}
        res.setResult(fs_connectivity, PTK_OKAY, "Success")
    except Exception as e:
        print "Failed", str(e)
    	res.setResult({'stacktype':stack['value'], 'components':components, 'connections':{}}, PTK_INTERNALERROR, "Failed to get cabling information")
    return res
