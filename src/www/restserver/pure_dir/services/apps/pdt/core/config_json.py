...
def export_configuration(stacktype, path=None):
    ...
    for shlv_file in glob.glob("*.shlv"):
	    ...
	    job_doc = None	
	    if job_recs['jobid'] is not None and os.path.exists(get_job_file(job_recs['jobid'])) is True:
	    ...
	    wf_doc = None
			...
			if wf_data[2] == True:   # Check for ovewrite Flag
                    		data_list.append(s)
	...
	#input_list.append({'README':"feel free to update 'value' field under global_config session"})
        for htype in htype_list:
		    ...
		    if ipt.getAttribute('name') in ['nexus_switch_a', 'nexus_switch_b', 'mds_switch_a', 'mds_switch_b', 'ucs_switch_a',
				'ucs_switch_b', 'pure_id', 'remote_file']:
    ...
    if os.path.exists(get_devices_wf_config_file()):
	    ...
	    if device.getAttribute('device_type') != 'PURE':
...
def get_component_description(tag, device_type, field):
   ...
   if field == 'array_ip':
	return "FlashArray IP Address " + editable_str
...
def prettfy_json(path, import_dict):
    ...
    tmp_dict['system_info']['stacktype'] = {'value' :tmp_dict['stacktype'], 'description':"FlashStack deployment type [type: read-only]" }
    tmp_dict['system_info']['version'] = {'value' :tmp_dict['version'], 'description':"Document version [type: read-only]" }	
    tmp_dict['system_info']['flags'] = {'value' :tmp_dict['flags'], 'description':"flags [type: read-only]" }
    tmp_dict['system_info']['tool_version'] =  {'value' :tmp_dict['tool_version'], 'description':"Generated Tool version [type: read-only]" }
	...
	tmp_dict['global_config' ][global_config['name'] ] =  {'value':global_config['value'],  'description': get_global_description(global_config['name'], global_config['description']) + " [type: editable]"}
	...
	if component['device_type'] == "PURE":
        ...
        comp_name = component['device_type'] + "_" + component['tag'] if component.get('tag', None) else component['device_type']
        tmp_dict ['components'] [comp_name] = {}
	for k, v in component.items():
		if k == 'tag' or k == 'device_type':
			continue
		tmp_dict ['components'] [comp_name][k] ={'value': v, 'description': get_component_description(component.get('tag', None), component['device_type'], k) }
	
    # add to global
    items= ['domain_name', 'dns', 'server_type', 'kvm_console_ip', 'esxi_file', 'esxi_kickstart']
    for item in items:
	tmp_dict['global_config' ][item] = {'value': tmp_dict['components']['UCSM_A'][item]['value'], 'description': tmp_dict['components']['UCSM_A'][item]['description']} 
    ...
    with open(path, 'w') as outfile:
		outfile.write("#README: Please do not edit fields, which are marked as [type: readonly] in description, Also please do not edit workflow section\n")
...
def undo_json_pretty(import_dict):
    ...
    tmp_dict['global_config'] = []
    tmp_dict ['components'] = []
    items= ['domain_name', 'dns', 'blade_image', 'server_type', 'kvm_console_ip', 'esxi_file', 'esxi_kickstart']
	...
	data = undo_json_pretty(tmp_data)
        for comp in data['components']:
            component = {
                ...
                "switch_name": comp['name'] if comp['device_type'] != 'PURE' else '',
                "tag": comp['tag'] if comp['device_type'] != 'PURE' else '',
                "switch_ip": comp['switch_ip'] if comp['device_type'] != 'PURE' else '',
            }
...
def add_componets_data(data, component, vals, addval):
    mds_fields = {'MDS System Image':'system_image', 'MDS Image version':'image_version', 'name':'name','MDS kickstart image' :'kickstart_image','IP Address':'switch_ip'}
    nexus_fields = {'Nexus 9k System Image': 'system_image', 'Nexus 9k Image version': 'image_version', 'name':'name', 'IP Address': 'switch_ip'}
    nexus_5k_fields = {'Nexus 5k System Image':'system_image','Nexus 5k Image version':'image_version', 'name':'name', 'Nexus 5k kickstart image':'kickstart_image','IP Address': 'switch_ip' }
    ucs_fields= {'UCS server type Blade/Rack': 'server_type','name':'name','UCS Blade firmware image':'blade_image','leadership':'leadership',
	'Nameserver IP':'dns', 'UCSM infra image':'infra_image', 'IP Address':'switch_ip','Virtual IP Address':'virtual_ip' }
    if vals != []:
    ...
    for val in vals:
	 ...
	 if component.startswith('MDS'):
		...
		key= ucs_fields[key.strip()]
	 	data['components'][component]['domain_name']={ 'value' : addval['domain_name'] } # is a work around fix it, bug
	 	data['components'][component]['dns']={ 'value' : addval['dns'] } # is a work around fix it, bug
	 else: 
    ...
    if component.startswith('UCSM'):
        data['components'][component]['blade_image'] = {'value': data['global_config']['firmware']['value']}
	if data['components'][component]['leadership']['value'] == 'primary':
                data['components'][component]['esxi_file'] = {'value': data['global_config']['remote_file']['value']}
...
def excel_to_json(stacktype,path):
    ...
    sheet = book[label]
    componenet_list = {'FlashArray':'FLASH ARRAY CONFIGURATION', 'MDS_A':'MDS A CONFIGURATION', 'MDS_B': 'MDS B CONFIGURATION' , 
		'Nexus 9k_A':'NEXUS(9K) A CONFIGURATION', 'Nexus 9k_B':'NEXUS(9K) B CONFIGURATION','UCSM_A':'UCSM A CONFIGURATION','UCSM_B':'UCSM B CONFIGURATION',
                'Nexus 5k_A':'NEXUS(5K) A CONFIGURATION', 'Nexus 5k_B':'NEXUS(5K) B CONFIGURATION' }
