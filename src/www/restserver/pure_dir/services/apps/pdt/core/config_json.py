...
def export_configuration(stacktype):
    ...
    for shlv_file in glob.glob("*.shlv"):
        ...
        for x in range(len(shelf['job'])):
            ...
            wfdict = {"wfname": job_recs['wfname'], "desc": job_recs['desc'],"wid": job_recs['wid']}
            tasklist = []
            for key, value in job_rec.iteritems():
                data_list=[]
                for k, v in value['inputs'].iteritems():
                    data = get_value_from_xml(job_recs['jobid'],value['texecid'],k)
                    s = {
                                 "name": k,
                                 "value":data[0],
                                 "mapval":data[1]
...
def import_configuration(configfile):
    ...
    if details[0]['stacktype'] != tmp_data['stacktype']:
        res.setResult(False, PTK_INTERNALERROR, "Upload Valid Stacktype JSON")
        return res
    shutil.copy2("/tmp/"+configfile.filename,get_download_path()+'import-'+tmp_data['stacktype']+'.json')
    ...
    for wfs in data['workflows']:
       for wf in wfs['subworkflows']:
           if len(wf['tasks']) >0:
               for i in wf['tasks']:
                   exec("%s = %s" % ("input_obj", i['taskid'] + "." + i['taskid'] + "Inputs" + "()"))
                   inputs = [x for x in dir(input_obj) if not x.startswith('__') and not x.endswith('__')]
                   for j in i['input']:
                        if j['name'] not in  inputs:
                            return j['name']
                        exec("%s = %s.%s" % ("field", "input_obj", j['name']))
                        wftaskip = job_task_inputs(field=field, tid=i['taskid'])
    ...
    if os.path.exists(get_download_path()+'import-'+stacktype+'.json') is True:
        ...
        for comp in data['components']:           
            ...
            if comp['device_type'] == 'Nexus 5k':
                component['switch_image'] = {"switch_system_image":comp['system_image'],"switch_kickstart_image":comp['kickstart_image']}
            elif comp['device_type'] == 'Nexus 9k':
                component['switch_image'] = {"switch_system_image":comp['system_image']}
    ...
    while 1:
        ...
        if status == True:
            if any(dev['configured'] == 'Re-validate' for dev in devices) == True:
                loginfo("%s configuration failed. Please retry the configuration" % dev['name'])
                return False

            conf_list = [dev['name'] for dev in devices if dev['configured'] == 'Configured']
            if len(devices) == len(conf_list):
                loginfo("Initial configuration completed for %s" % str(conf_list))
                init = 1
                break
            else:
                loginfo("Waiting for all devices to be configured, current:" + str(len(conf_list)))
                time.sleep(5)
...
def restore_config(stacktype, datas):
    ...
    if res_1.getStatus() == PTK_OKAY:
        ...
        if res_2.getStatus() == PTK_OKAY:
            threading.Thread(target=deploy_workflows, args=(stacktype,)).start()
            res.setResult([], PTK_OKAY, "success")
        else:
            res.setResult([], PTK_INTERNALERROR, "Component Initialization Failed")
    else:
        res.setResult(res_1.getResult(), PTK_INTERNALERROR, "Failed to save the configuration details")
    return res
...
def update_config_inputs(stacktype, jid):
    ...
    for jwf in wf_list:
        if '@id' in jobdoc['workflow'] and jobdoc['workflow']['@id'] == jwf['wid']:
            taskset = [(xtask, jtask) for xtask in jobdoc['workflow']['tasks']['task'] for jtask in jwf['tasks'] if xtask['@texecid'] == jtask['texecid'] ]
            for task in taskset:
                task_dict = {}
                xml_task, json_task = task[0], task[1]
                xml_args = [xml_task['args']['arg']] if type(xml_task['args']['arg']) != list else xml_task['args']['arg']
                argset = [(xarg, jarg) for xarg in xml_args for jarg in json_task['input'] if xarg['@name'] == jarg['name'] ]
                for arg in argset:
                    arg_dict = {}
                ...
                loginfo("Updating task id %s of workflow %s" % (json_task['texecid'].encode('utf-8'), jwf['wid']))
                job_task_input_save_api(jid, json_task['texecid'], task_dict, "job")
    return True
