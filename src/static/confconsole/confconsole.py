...
class TurnkeyConsole:
    ...
    def __init__(self, pluginManager, eventManager, advanced_enabled=True):
        title = "Flash Stack Deployment Configuration Console"
        self.width = 60
        self.height = 20
	...
	def _validate(start, end, netmask, gateway, nameservers):
	    ...
	    if not start:
                ...
                errors.append("Invalid DHCP Start Range IP address: %s" % start)
	...
	while 1:
            ...
            retcode, input = self.console.form("DHCP Server settings", text, fields)
